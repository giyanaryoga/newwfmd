...
public class GenerateMeAccessDao {
    ...
    public JSONArray callGenerateMeAccess(String wonum) throws SQLException, JSONException {
        String deviceLink1 = getAssetattridType(wonum).get("DEVICELINK").toString();
//        String deviceLink2 = getAssetattridType(wonum).get("LINK_TYPE").toString();
        try {
            String url = "https://api-emas.telkom.co.id:8443/api/device/linkedPort?" + "deviceName=" + getAssetattridType(wonum).get("AN_NAME").toString().replace("/", "%2F").replace(" ", "%20") + "&portName=" + getAssetattridType(wonum).get("AN_UPLINK_PORTNAME").toString().replace("/", "%2F").replace(" ", "%20") + "&deviceLink=" + deviceLink1;
